...
class ArrivalDateService {
   ...
   */
  static calculateArrivalDate(distance, shippingMethod = 'domestic', startDate = new Date()) {
    if (!distance || distance <= 0) {
      return null;
    }

    const method = shippingMethod.toLowerCase();
    const transitConfig = this.TRANSIT_TIMES[method] || this.TRANSIT_TIMES.domestic;
  ...
  static getTransitTimeEstimate(distance, shippingMethod = 'domestic') {
    ...
    const minDays = Math.max(1, Math.floor(transitConfig.base + (distance * transitConfig.perKm * 0.8)));
    const maxDays = Math.max(minDays + 1, Math.ceil(transitConfig.base + (distance * transitConfig.perKm * 1.2)));
   ...
   * @returns {string}
   */
  static determineShippingMethod(pickupCountry, deliveryCountry, preferredMethod = 'air') {
