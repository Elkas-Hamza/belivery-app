...
import { FaTimes, FaBox, FaMapMarkedAlt, FaPhone, FaWeight, FaMoneyBillWave, FaClipboardList, FaUser, FaCalendarAlt, FaBarcode, FaPrint } from 'react-icons/fa';
...
const DeliveryDetailsModal = ({ deliveryId, onClose }) => {
    ...
    try {
      ...
      if (error.response) {
        ...
        if (error.response.status === 401) {
          ...
          setError('Delivery not found. It may have been deleted or you may not have permission to view it.');
        } else if (error.response.status === 500) {
          setError('Server error. Please try again later or contact support if the problem persists.');
        } else if (error.response.data && error.response.data.message) {
          setError(error.response.data.message);
        } else {
          setError(`Error ${error.response.status}: Could not load delivery information`);
        }
      ...
      <div className="modal-overlay">
        <div className="modal-container">
          ...
          <div className="modal-content error">
            ...
            <button onClick={fetchDeliveryDetails} className="retry-btn">Retry</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-container delivery-details" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
        ...
        <div className="modal-content">
            ...
            <div className="timeline">
              <div className="timeline-item">
                ...
                <div className="timeline-content">
                  <p className="timeline-time">{formatDate(delivery.created_at)}</p>
              ...
              {delivery.status === 'in_progress' || delivery.status === 'delivered' ? (
                <div className="timeline-item">
                  <div className="timeline-icon completed"></div>
                  <div className="timeline-content">
                    <p className="timeline-time">{formatDate(new Date(new Date(delivery.created_at).getTime() + 60*60*1000))}</p>
              ...
              {delivery.status === 'delivered' ? (
                <div className="timeline-item">
                  <div className="timeline-icon completed"></div>
                  <div className="timeline-content">
                    <p className="timeline-time">{formatDate(new Date(new Date(delivery.created_at).getTime() + 3*60*60*1000))}</p>
                ...
                <div className="timeline-item">
                  ...
                  <div className="timeline-content">
                    <p className="timeline-time">{formatDate(new Date(new Date(delivery.created_at).getTime() + 30*60*1000))}</p>
                ...
                <div className="timeline-item">
                  <div className="timeline-icon pending"></div>
                  <div className="timeline-content">
                    <p className="timeline-time">Estimated: {formatDate(new Date(new Date(delivery.created_at).getTime() + 3*60*60*1000))}</p>
          ...
          <div className="modal-actions">
            ...
            <button className="secondary-btn" onClick={() => setShowPrintLabel(true)}>
